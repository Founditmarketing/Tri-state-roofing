    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8">
      ...
      <div>
        ...
        <ul className="space-y-2 text-sm">
          {PAGE_SERVICES.map((s) => (
            <li key={s.id}>
              <Link to={`/services/${s.slug}`} className="hover:text-white transition-colors">{s.service.title}</Link>
            </li>
          ))}
        </ul>
      </div>
      <div>
        ...
        <ul className="space-y-2 text-sm">
          {LOCATIONS.map((loc) => (
            <li key={loc.slug}>
              <Link to={`/locations/${loc.slug}`} className="hover:text-white transition-colors">{loc.city}, {loc.state}</Link>
            </li>
          ))}
        </ul>
      </div>
